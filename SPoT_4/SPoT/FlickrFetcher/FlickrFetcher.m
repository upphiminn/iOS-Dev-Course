...
+ (NSArray*)removePhotosIn:(NSArray*) photos withTags:(NSSet*) tagsToRemove //not used
{
    NSMutableArray *filteredPhotos = [[NSMutableArray alloc] init];
    for (NSDictionary* photo in photos)
    {
        NSArray *photoTags = [photo[FLICKR_TAGS] componentsSeparatedByString:@" "];
        NSMutableSet *photoTagSet = [NSMutableSet setWithArray: photoTags];
        [photoTagSet intersectSet: tagsToRemove];
        NSArray *resultArray = [photoTagSet allObjects];
...
+ (NSSet*) getTagsForPhotos:(NSArray*) photos
{
    NSMutableSet* uniqueTags = [[NSMutableSet alloc] init];
    for (NSDictionary* photo in photos){
        NSArray* photoTags = [photo[FLICKR_TAGS] componentsSeparatedByString:@" "];
        [uniqueTags addObjectsFromArray:photoTags];
    }
...
+ (NSDictionary*) groupPhotos:(NSArray*)photos byTags:(NSArray*) tags
{
    NSMutableDictionary* photosByTags = [[NSMutableDictionary alloc] init];
    for(NSDictionary* photo in photos){
        NSArray* photoTags = [photo[FLICKR_TAGS] componentsSeparatedByString:@" "];
        for(NSString* tagString in photoTags){
            if(!photosByTags[tagString]){
                NSMutableArray* photoList = [[ NSMutableArray alloc] init];
                [photoList addObject:photo];
                [photosByTags setObject:photoList forKey:(id)tagString];
            }
            else{
                NSMutableArray* photoList = [photosByTags objectForKey:tagString];
                [photoList addObject:photo];
                [photosByTags setObject:photoList forKey:(id)tagString];
            }
